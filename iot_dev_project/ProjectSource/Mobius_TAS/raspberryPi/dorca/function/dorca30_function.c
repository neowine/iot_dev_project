...
int eep_page_read(unsigned int msb, unsigned int lsb,int compare, unsigned char *pData)
{
	...
	printf("\r\n TRSRST");
	{
		printf("\r\n addr =%02x%02x",msb,lsb);
		printf("\r\n================read data================");
		printf("\r\n spi_rx_data    :"); for ( i=0; i<64; i++ ){ if ( ( i !=0 ) & ( i % 16 == 0 ) ) printf("\n                 "); printf(" 0x%02x", rx_data[i]); } 
...
L_MIDR_START:
	while(1)
	{
		...
		printf("\r\n  * MIDR_CNT0 =");
		for( i = 7; i >= 0; i--)
			printf("%02x ",MIDRCNT0[i]);

		printf("\r\n  * MIDR_CNT1 =");
		for( i = 7; i >= 0; i--)
			printf("%02x ",MIDRCNT1[i]);
...
int GetSuperWirePermission(void)
{
	...
	unsigned char temp_addr[2];
	int k = 0;
	unsigned char SUPER_WIRE_PW[16];
	//return;
	for(i = 0; i < 64; i++)
	{
		tx_data[i] = 0;
...
#if 1
#if 0 // CASE 1 KEYLOADë¥??ë²ˆ ?˜í–‰?˜ë©´ UID PERM?»ì–´ì§€ê³?AES_CUR??1?´ëœ??
	//	printf("\r\n -----------------------------SetKEYNormal------------------------------");
...
#endif
	//SetKEYNormal();
#if 0// CASE 2 PERMISSION???œë²ˆ ?»ì–´?¤ë©´ ???
	GetPermissionByPW(UID_PW_CT, RG_PERM_UID_PASS);
	ReleasePermision();
...
int RSCreate12(void)
{
	...
	//	Delay_ms(6000);
	//////{{ ADD 2017-08-10 LEDë¶ˆì„ ì¼°ë‹¤ ?„ê¸°?„í•œ ì½”ë“œ 
...
int PrintBISTResult(int BistNum,int retVal)
{
	...
	if(retVal != 0x80)
	{
		printf("FAIL  value 0x%02x",retVal);
		success = 0;
	}
	else
		printf("PASS");
...
int TAESM241_S()
{
	...
	unsigned char DEC_CYPER_TEXT_REV[16*4];
	unsigned char IV_REV[16];	
	unsigned char KEY32[32] ;//?œê??…ë ¥ ê¹”ë³´ê´? 
	printf("\r\n ================================== TAESM241_S ================================== ");
	memset(KEY32,0,32);
	hexstr2bytes("1f352c073b6108d72d9810a30914dff4", KEY32);
	hexstr2bytes("603deb1015ca71be2b73aef0857d7781", KEY32+16);	
...
//TO verify result change h value
	...
	buffer_ecdsa[3] = 0;
	buffer_ecdsa[4] = 32;
	hexstr2bytes("0000000000000000000000000f7b55549fab573c0361b832ad0be8cdeef91b51",&buffer_ecdsa[5]);
	send_data_arm7(buffer_ecdsa,37);         
	delay_ms(40);	
	delay_ms(200);
	buffer_ecdsa[0] = SPI1_WRITE_DATA;
	...
	printf("\r\n Expected\r\n");
	printf("d6606271131e7e7e617a81aa11f09e7ed56311828823367a869b454040b3f905");
	...
	printbyte2(buffer_receive,32); 
	printf("\r\n Expected\r\n");
	printf("b5b417619bf9fa89d50b3e22782a2de80a86db67e728114e6e0e91cab1a41612");
	...
	printbyte2(buffer_receive,32);
	printf("\r\n Expected\r\n");
	printf("e43e8111258bea6f5c96bd6d66715748fbee756da418de90f64066c6b3e072f1");
...
void AES_ARIA_Encrypt(unsigned char *pInput,unsigned char *pOutput)
{
	...
	unsigned char rx_data[64];
	reversebuffer(tx_data, pInput, 16);
	tspi_interface(cs, ADDR_NOR_W, RG_EEBUF300      , NULL, NULL, NULL, NULL, tx_data, rx_data, 16);	//TODO
	delay_us(2);
	tspi_interface(cs, ADDR_NOR_R, RG_EEBUF320      , NULL, NULL, NULL, NULL, tx_data, rx_data, 16);		
	//iEnd = pRSTC->RTTC_RTVR;
	reversebuffer(pOutput, rx_data, 16);	
...
void Gurum_TEST_MAIN()
{
		...
		while(1)
		{
			...
			printf("\r\n  * 		   GURUM	  TEST MAIN 					 *");
			printf("\r\n  *****************************************************");
			printf("\r\n  * 1. AES ARIA OPERATION MODE TEST					*");	
			printf("\r\n  * 2. ecdh_gen_pub_key					  		*");
