...
bool TasCCS811::begin(uint8_t addr){
	_i2caddr = addr;
...
void TasCCS811::setDriveMode(uint8_t mode){
	_meas_mode.DRIVE_MODE = mode;
	this->write8(CCS811_MEAS_MODE, _meas_mode.get());
}

void TasCCS811::enableInterrupt(){
	_meas_mode.INT_DATARDY = 1;
	this->write8(CCS811_MEAS_MODE, _meas_mode.get());
}

void TasCCS811::disableInterrupt(){
	_meas_mode.INT_DATARDY = 0;
	this->write8(CCS811_MEAS_MODE, _meas_mode.get());
}

bool TasCCS811::available(){
...
void TasCCS811::setEnvironmentalData(uint8_t humidity, double temperature){
...
double TasCCS811::calculateTemperature(){
	uint8_t buf[4];
...
void TasCCS811::setThresholds(uint16_t low_med, uint16_t med_high, uint8_t hysteresis){
...
void TasCCS811::SWReset(){
	//reset sequence from the datasheet
	uint8_t seq[] = {0x11, 0xE5, 0x72, 0x8A};
	this->write(CCS811_SW_RESET, seq, 4);
}

bool TasCCS811::checkError(){
	_status.set(read8(CCS811_STATUS));
	return _status.ERROR;
}

void TasCCS811::write8(byte reg, byte value){
	this->write(reg, &value, 1);
}

uint8_t TasCCS811::read8(byte reg){
	uint8_t ret;
	this->read(reg, &ret, 1);
...
void TasCCS811::_i2c_init(){
	Wire.begin();
}

void TasCCS811::read(uint8_t reg, uint8_t *buf, uint8_t num){
	uint8_t value;
	uint8_t pos = 0;
...
void TasCCS811::write(uint8_t reg, uint8_t *buf, uint8_t num){
	Wire.beginTransmission((uint8_t)_i2caddr);
